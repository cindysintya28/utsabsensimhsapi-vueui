<template>
  <div class="home">
    <Slider />
    <hr class="my-3">
      <router-link class="btn btn-primary" to="/createabsens">Add absen</router-link>
    <table class="table">
  <thead>
    <tr>
       <th scope="col">Id</th>
  ...
  <tbody>
    <tr v-for="(absen, index) in absens" :key="index">
      ...
      <td>
        <router-link class="btn btn-success" :to="{name:'Editabsens', params:
        {id:absen.id}}"
          >Edit</router-link>
        <button @click.prevent="absenDelete(absen.id)" class="btn 
